...
def daily_refresh():
    ...
    if alerts:
        ...
        print(f"  ⚠️  {len(alerts)} complaint spike alerts generated")
    else:
...
def monthly_refresh():
    ...
    if all_alerts:
        ...
        print(f"  ⚠️  {len(all_alerts)} monthly alerts generated")
    else:
    ...
    result = os.system("python process_data.py")
    if result == 0:
...
def quarterly_refresh():
    ...
    if all_alerts:
        ...
        print(f"  ⚠️  {len(all_alerts)} quarterly alerts generated")
...
def run_scheduler():
    ...
    Consider Replit Always On ($7/mo) or deploy to Railway/Render.
    """
...
def test_connection():
    ...
    if ANTHROPIC_KEY:
        ...
        print("\n  ⚠️  ANTHROPIC_API_KEY not set — skipping AI test")
