...
Serves index.html and all static files on port 3000.
...
def main():
    ...
    for f in optional_files:
        exists = os.path.exists(f)
        status = "✅" if exists else "⚠️  not found (optional)"
        print(f"    {status} {f}")

    if not all_ok:
        print("\n  ⚠️  Some required files are missing.")
        print("  Run python process_data.py first to generate scores.\n")

    # Check environment variables
    print("\n  Checking secrets:")
    nyc_token = os.environ.get("NYC_OPEN_DATA_TOKEN", "")
    ant_key   = os.environ.get("ANTHROPIC_API_KEY", "")
    print(f"    {'✅' if nyc_token else '⚠️ '} NYC_OPEN_DATA_TOKEN {'set' if nyc_token else 'not set — data pipeline disabled'}")
    print(f"    {'✅' if ant_key   else '⚠️ '} ANTHROPIC_API_KEY   {'set' if ant_key   else 'not set — AI chat uses fallback responses'}")
